    ...
    constructor(props) {
        ...
        this.state = {
            ...
            collect_hdao: async (from, swap_id, token_address, token_id) => {
                ...
                let marketplace = await Tezos.wallet.at(
            ...
            swapv2: async (
                ...
                const objktsAddress =
                    this.state.proxyAddress || this.state.objkts
                const marketplaceAddress =
                    this.state.proxyAddress || this.state.v2
            ...
            batch_cancel: async (arr) => {
                ...
                const batch = await arr
                    .map((e) => parseInt(e.id))
                    .reduce((batch, id) => {
                        return batch.withContractCall(
                            v1.methods.cancel_swap(id)
                        )
                    }, Tezos.wallet.batch())
                return await batch.send()
            },
            ...
            setFeedback: (props) =>
                this.setState({
                    feedback: { ...this.state.feedback, ...props },
                }),
      ...
      */
            ...
            getBalance: (address) => {
                axios
                    .get(
                        `https://api.tzkt.io/v1/accounts/${address}/balance_history`,
                        {
                            params: {
                                address: address,
                            },
            ...
            findOriginatedContractFromOpHash: async (hash) => {
                ...
                axios
                    .get(
                        `https://api.tzkt.io/v1/operations/originations/${hash}`
                    )
                    .then((response) => {
                        const { data } = response
