...
export default class Display extends Component {
        ...
        if (id === 'tz') {
            ...
            try {
                if (res[0]) {
                    ...
                    if (meta.description)
                        this.setState({ description: meta.description })
                    if (meta.identicon)
                        this.setState({ identicon: meta.identicon })
                    if (res[0]) this.setState({ subjkt: res[0].name })
                    if (res[0])
                        this.setState({
                            hdao: Math.floor(res[0].hdao_balance / 1000000),
                        })
                }
            } catch (e) {
                console.log('error ' + e)
            ...
            if (res.length >= 1) {
                ...
                console.log(resTz)
                this.setState({
                    hdao: Math.floor(resTz[0].hdao_balance / 1000000),
                })
            } else {
    ...
    creationsForSale = async (forSaleType) => {
        ...
        if (forSaleType !== null) {
            if (forSaleType == 0) {
                this.setState({
                    objkts: await this.filterCreationsForSalePrimary(
                        this.state.objkts
                    ),
                })
            } else if (forSaleType == 1) {
                this.setState({
                    objkts: await this.filterCreationsForSaleSecondary(
                        this.state.objkts
                    ),
                })
            }
    ...
    filterCreationsForSaleSecondary = async () => {
        let objkts = this.state.creations.filter((item) => {
            const swaps = item.swaps.filter((swaps) => {
                return (
                    swaps.status == 0 && swaps.creator_id !== this.state.wallet
                )
            })
            return swaps && swaps.length > 0
        })
    ...
    collectionFull = async () => {
        ...
        if (!list.includes(this.state.wallet)) {
            ...
            // console.log(swaps)
            let combinedCollection = await this.combineCollection(
                collection,
                swaps
            )
                    ...
                    <Padding>
                        <div className={styles.profile}>
                            ...
                            <div className={styles.info}>
                                ...
                                >
                                    <Primary>
                                        {walletPreview(this.state.wallet)}
                                    </Primary>
                                </Button>
                        ...
                        <Container xlarge>
                            {this.state.collectionType == 'forSale' ? (
                                <>
                                    ...
                                    this.context.acc.address ==
                                        this.state.wallet ? (
                                        <>
                                            {Object.keys(this.state.marketV1)
                                                .length !== 0 && (
                                                <>
                                                    <Container>
                                                        <Padding>
